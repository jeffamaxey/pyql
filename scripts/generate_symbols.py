...
def symbol_generator_from_obj_file(object_file):
    ...
    for line in six.text_type(nm_result, 'ascii').split('\n'):
        ...
        if len(mangled_symbol) > 0 and (
            _is_ql_symbols(mangled_symbol)
            or _is_boost_assertion(mangled_symbol)
        ):
            yield mangled_symbol
            
def process_directory(directory_name):
    for object_file in glob.glob(os.path.join(directory_name, '*.obj')):
        yield from symbol_generator_from_obj_file(object_file)
...
def test():
    ...
    for _ in symbol_generator_from_obj_file(test_file):
        pass
